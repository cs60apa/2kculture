...
export default function AlbumsPage() {
  ...
  const toggleAlbumPublicationStatus = useMutation(api.music.toggleAlbumPublicationStatus);
  ...
  const songsByAlbum = useQuery(
    api.music.getSongsByArtist,
    isLoaded && isSignedIn ? { artistId: user?.id } : "skip"
  ) || [];
  
  // Group songs by album
  const albumSongs = albums.reduce((acc: Record<string, any[]>, album: any) => {
    acc[album._id] = songsByAlbum.filter((song: any) => song.albumId === album._id);
    return acc;
  }, {});
  ...
  const handleTogglePublicationStatus = async (album: any) => {
      ...
      <div className="space-y-8">
        ...
        {albums.length === 0 ? (
            ...
            <CardContent>
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                  Create your first album and organize your songs into a cohesive listening experience.
                </p>
                <Button onClick={() => router.push('/studio/upload')}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {albums.map((album) => {
              const { totalPlays, totalLikes, songCount } = getAlbumStats(album._id);
              ...
              return (
                <Card key={album._id} className="overflow-hidden">
                  <div className="h-48 relative bg-muted">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => router.push(`/studio/albums/${album._id}`)}>
                  ...
                  <CardHeader>
                    <CardTitle className="text-xl truncate">{album.title}</CardTitle>
                    <CardDescription>{formatDate(album.releaseDate)}</CardDescription>
                  </CardHeader>
                  <CardContent>
                  ...
                  <CardFooter className="flex justify-between">
                    <Button variant="outline" size="sm" onClick={() => router.push(`/studio/albums/${album._id}`)}>
                      <Music className="mr-2 h-4 w-4" /> Manage Songs
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => router.push(`/studio/analytics?albumId=${album._id}`)}>
